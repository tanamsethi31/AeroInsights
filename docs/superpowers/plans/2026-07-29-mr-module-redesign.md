...
> **Post-hoc correction (after 3 review rounds):** the "out of scope" framing above didn't survive contact with the task's own diff. Multiplying by `heuristicEventCost` (a large $ figure) instead of the old small per-unit `rateAmount` turned the dormant `intervalFH:0` quirk into an active, severe bug — first a −$866M blowup for LLPs, then (after a `[0,1]` clamp) a silent, permanent $0 for LLPs on every lease. The actual fix (commit `8dcdbad`) drops the `h?.intervalFH ??` reference entirely and uses `comp.fullIntervalUnits` directly — the same reliable, basis-correct field `currentObligation` already used — making `eolObligation` and `currentObligation` fully symmetric. Correctly deviating from this task's own literal "don't touch it" instruction was the right call once the interaction became clear; noting it here so a future reader understands why.
