# Sprint 8 — F13: Lessee Deep-Dive Profile Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the Counterparties page right-column with a self-contained `LesseeProfilePanel` component that shows a 5-tab profile (Overview · Leases · ECL · Timeline · Scenarios) for the selected lessee.

**Architecture:** New `LesseeProfilePanel.tsx` under `src/app/components/counterparties/` holds all types, synthetic datasets for 6 lessees, helpers, and all tab UI. `Counterparties.tsx` is stripped down to sidebar list + `<LesseeProfilePanel lesseeId={selectedLessee.id} />`. Follows the same isolation pattern as `ConcentrationTab.tsx`.

**Tech Stack:** React 18, TypeScript, Vite, Recharts (`BarChart`, `Bar`, `Cell`, `XAxis`, `YAxis`, `CartesianGrid`, `Tooltip`, `ResponsiveContainer`, `ReferenceLine`, `RadarChart`, `PolarGrid`, `PolarAngleAxis`, `Radar`), existing `KpiCard`, `StatusPill`, `Card` UI components, inline `style={{}}`.

---

### Task 1: Types, all 6-lessee datasets, helpers, and stub export

**Files:**
- Create: `src/app/components/counterparties/LesseeProfilePanel.tsx`

**Context:** No test runner — verification is `npm run build` from `/Users/tanamsethi/Downloads/Aeroinsights`. The `counterparties/` directory does not yet exist — create it. All dollar amounts are stored as raw USD (multiply by `M = 1_000_000` or `K = 1_000` inline). The `makeDPD` helper pairs 12 numeric DPD values with the month labels `["May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar","Apr"]`.

- [ ] **Step 1: Create the directory and file**

```bash
mkdir -p /Users/tanamsethi/Downloads/Aeroinsights/src/app/components/counterparties
```

Then create `/Users/tanamsethi/Downloads/Aeroinsights/src/app/components/counterparties/LesseeProfilePanel.tsx` with this exact content:
...
git commit -m "feat(sprint8): add LesseeProfilePanel types, 6-lessee datasets, helpers, stub"
```

---

### Task 2: Full 5-tab component — profile header, Overview, Leases, ECL, Timeline, Scenarios

**Files:**
- Modify: `src/app/components/counterparties/LesseeProfilePanel.tsx`

**Context:** Replace everything from `// ─── LesseeProfilePanel (stub...)` to end of file with the full implementation. Types, data, and helpers above that line are untouched.

- [ ] **Step 1: Replace the stub section with the full component**

Remove everything from `// ─── LesseeProfilePanel (stub — full UI added in Task 2) ─────────────────────` to end of file, and replace with:
...
```bash
git add src/app/components/counterparties/LesseeProfilePanel.tsx
git commit -m "feat(sprint8): add full LesseeProfilePanel — 5 tabs, profile header, radar, DPD chart, ECL bar chart, scenario sensitivity"
```

---

### Task 3: Wire LesseeProfilePanel into Counterparties.tsx

**Files:**
- Modify: `src/app/pages/Counterparties.tsx`

**Context:** `Counterparties.tsx` currently has these items in its right column that move into `LesseeProfilePanel`: the profile header `<div>`, two `<KpiCard>` components, the `<Card>` with radar chart, and the Restructuring Simulator `<Card>`. The `restructuringOptions` constant and `showRestructuring` state also move out. The recharts import (`RadarChart, PolarGrid, PolarAngleAxis, Radar, ResponsiveContainer`) becomes unused and must be removed. The `lessees` array and `selectedLessee` state stay — they drive the sidebar list.

- [ ] **Step 1: Add the import**

At the top of `/Users/tanamsethi/Downloads/Aeroinsights/src/app/pages/Counterparties.tsx`, add after the last import line:

```tsx
import { LesseeProfilePanel, type LesseeId } from "../components/counterparties/LesseeProfilePanel";
```

- [ ] **Step 2: Remove the recharts import**
...
- [ ] **Step 3: Remove restructuringOptions constant**

Remove the entire `restructuringOptions` array (lines starting `const restructuringOptions = [` through its closing `];`).

- [ ] **Step 4: Remove showRestructuring state and update the component signature**

In the `Counterparties` function body, remove:

```tsx
const [showRestructuring, setShowRestructuring] = useState(false);
```

Also update the `setSelectedLessee` call in the sidebar button's `onClick` to remove the `setShowRestructuring(false)` call:
...
- [ ] **Step 5: Replace the right-column Lessee Detail div**

Find the entire `{/* Lessee Detail */}` block — it starts with:
```tsx
        {/* Lessee Detail */}
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
```
and ends with the closing `</div>` of that flex column (just before the final `</div>` that closes the grid, then `</div>` closing the page, then `);` closing the return).
...
If you see "Cannot find name 'RadarChart'" or similar, the recharts import line was not fully removed — delete it.
If you see "showRestructuring is not defined", there is a remaining reference in the JSX — search for `showRestructuring` and remove it.

- [ ] **Step 7: Commit**

```bash
git add src/app/pages/Counterparties.tsx
git commit -m "feat(sprint8): wire LesseeProfilePanel into Counterparties, remove old right-column charts and restructuring state"
```
...
**Spec coverage:**
- ✅ 5-tab structure: Overview · Leases · ECL · Timeline · Scenarios
- ✅ Profile header: dark `#002147` banner with name, country, rating, leases, exposure, stage pill
- ✅ Overview: analyst note, behavior score KPI, days-overdue KPI, radar chart + score bars, restructuring simulator (7 options, migrated from Counterparties.tsx)
- ✅ Leases: 3-KPI strip (leases, total rent, WA remaining term EAD-weighted), lease table (7 columns)
- ✅ ECL: 4-KPI strip (ECL 12m, ECL LT, ECL rate, WA PD), ECL decomposition table (9 columns), horizontal bar chart coloured by stage
- ✅ Timeline: 12-month DPD bar chart with `ReferenceLine y={30}` trigger line, event log table with colour-coded type badges
- ✅ Scenarios: 3-row sensitivity table with row tinting (white/amber/red), Δ vs Base column
- ✅ All 6 lessees have complete datasets
- ✅ `useEffect` resets active tab to Overview on lesseeId change
- ✅ `waRemainingTerm` computed from `2026-05-02` relative to leaseEnd dates
- ✅ Counterparties.tsx stripped of recharts, restructuringOptions, showRestructuring

**Type consistency:**
- `LesseeId` union used consistently across `PROFILE_DATA` keys, `LesseeProfilePanel` prop, and the cast in `Counterparties.tsx`
- `LesseeMeta`, `LesseeLeaseRow`, `LesseeECLRow`, `MonthlyDPD`, `PaymentEvent`, `ScenarioRow` defined in Task 1 and referenced without redefinition in Task 2
- `eventColour(ev.type)` returns a hex string; the `${eventColour(...)}18` and `${eventColour(...)}30` hex-alpha concatenations work because all palette colours are 6-digit hex
- `stageColour(r.stage)` used in both ECL bar chart `Cell` and nowhere else — consistent

**No placeholders:** All code complete and explicit. ✅
