# P1-F1: Data Import Wizard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a 5-step guided import wizard (modal) accessible from Settings > Data Sources, with client-side validation, preview, ECL simulation, and a post-import Dashboard onboarding checklist.

**Architecture:** A full-screen modal `ImportWizard` component holds all five step sub-components. Client-side XLSX parsing uses the `xlsx` library (already installed). Onboarding state (checklist completion, dismissed flag) lives in a new `OnboardingContext` with localStorage persistence. The Dashboard conditionally renders a checklist widget by reading that context.

**Tech Stack:** React 18, TypeScript, `xlsx` ^0.18.5 (SheetJS), `lucide-react`, inline CSS (no Tailwind classes in components — follow existing codebase pattern), React Router `useNavigate`, localStorage for persistence.
...
|----------|------------------------------------------------------------|------------------------------------------------------------|
| Create   | `src/app/contexts/OnboardingContext.tsx`                   | Checklist state + localStorage persistence + provider     |
| Create   | `src/app/components/import/ImportWizard.tsx`               | 5-step modal: template, upload, validate, preview, confirm |
| Modify   | `src/app/pages/Settings.tsx`                               | Add "Import New Portfolio" button + mount wizard           |
| Modify   | `src/app/pages/Dashboard.tsx`                              | Onboarding checklist widget (conditional, dismissable)     |
| Modify   | `src/app/App.tsx`                                          | Wrap tree with `<OnboardingProvider>`                      |
...
**Files:**
- Create: `src/app/contexts/OnboardingContext.tsx`
...
```bash
git add src/app/contexts/OnboardingContext.tsx
git commit -m "feat: add OnboardingContext with checklist state and localStorage persistence"
```

---

## Task 2: ImportWizard — shared types, styles, and Step Indicator

**Files:**
- Create: `src/app/components/import/ImportWizard.tsx` (initial skeleton)

- [ ] **Step 1: Create the file with shared types, button styles, and StepIndicator**
...
// ─── Placeholder exports (steps added in subsequent tasks) ────────────────────
// (Root wizard shell added in Task 7)
export function ImportWizard(_props: { onClose: () => void }) {
  return <div>ImportWizard placeholder</div>;
}
```
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: scaffold ImportWizard with shared types, styles, and StepIndicator"
```

---

## Task 3: ImportWizard — Step 1 (Template Download)

**Files:**
- Modify: `src/app/components/import/ImportWizard.tsx`

- [ ] **Step 1: Add the template-generation function and Step1 component**

Add the following directly before the `export function ImportWizard` placeholder at the bottom of `ImportWizard.tsx`. Replace the placeholder export with the complete content in Task 7.
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div>
        ...
        <p style={{ fontSize: "0.875rem", color: "#64748B", margin: 0, lineHeight: 1.6 }}>
          The template is an XLSX workbook with 6 sheets. Fill in your data, then
          upload it in the next step. Only the <strong>Leases</strong> sheet is
          required to start — the others enrich the analysis.
        </p>
      </div>
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: add ImportWizard Step 1 — template download with 6-sheet XLSX generation"
```

---

## Task 4: ImportWizard — Step 2 (Upload)

**Files:**
- Modify: `src/app/components/import/ImportWizard.tsx`

- [ ] **Step 1: Add Step2 component after Step1, before the placeholder export**
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: add ImportWizard Step 2 — drag-and-drop file upload"
```

---

## Task 5: ImportWizard — Step 3 (Validate) with inline editor

**Files:**
- Modify: `src/app/components/import/ImportWizard.tsx`

- [ ] **Step 1: Add the client-side validation logic and Step3 component**

Add after Step2, before the placeholder export:
...
async function parseAndValidate(file: File): Promise<ValidationResult> {
  ...
  const sheetName = wb.SheetNames.includes("Leases")
    ? "Leases"
    : wb.SheetNames[0];
...
function Step3({
  ...
  const canProceed =
    phase.phase === "done" && phase.result.errors.length === 0;
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: add ImportWizard Step 3 — client-side validation with inline error editor"
```

---

## Task 6: ImportWizard — Step 4 (Preview) and Step 5 (Confirm + ECL simulation)

**Files:**
- Modify: `src/app/components/import/ImportWizard.tsx`

- [ ] **Step 1: Add Step4 (Preview) after Step3**
...
- [ ] **Step 2: Add Step5 (Confirm + ECL simulation) after Step4**
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: add ImportWizard Steps 4 & 5 — data preview and ECL simulation with onboarding trigger"
```

---

## Task 7: ImportWizard — Root Shell (wire all 5 steps)

**Files:**
- Modify: `src/app/components/import/ImportWizard.tsx`

- [ ] **Step 1: Replace the placeholder `export function ImportWizard` at the bottom of the file**
...
```bash
git add src/app/components/import/ImportWizard.tsx
git commit -m "feat: wire ImportWizard root shell — all 5 steps connected"
```

---

## Task 8: Settings.tsx — Add "Import New Portfolio" button

**Files:**
- Modify: `src/app/pages/Settings.tsx`

- [ ] **Step 1: Add import and state for wizard**

In `Settings.tsx`, add the import at the top (after the existing imports):

```typescript
import { ImportWizard } from "../components/import/ImportWizard";
```

Add `showImportWizard` state inside the `Settings()` function body, directly after the `[saved, setSaved]` state line:

```typescript
const [showImportWizard, setShowImportWizard] = useState(false);
```

- [ ] **Step 2: Update the Data Sources `Card` to show the button and conditionally render the wizard**

Find this exact block in `Settings.tsx`:
...
Then find the closing of the `datasources` block:
...
Also add `Upload` to the lucide-react import — find the existing lucide import line and add `Upload` if not already present. The current line is:

```typescript
import { Building2, Users, Database, Sliders, ClipboardList, Save, Plus, Trash2, Eye, EyeOff, Check, Bell, Mail, X } from "lucide-react";
```
...
```
1. Navigate to http://localhost:5176/settings/data-sources
2. Verify "Import New Portfolio" button appears in the Data Sources card header (top-right)
3. Click the button — wizard modal should open with Step 1 (Template Download)
4. Click the X or backdrop — modal should close
```
...
git commit -m "feat: add Import New Portfolio button to Settings > Data Sources"
```

---

## Task 9: Dashboard.tsx — Onboarding checklist widget

**Files:**
- Modify: `src/app/pages/Dashboard.tsx`

- [ ] **Step 1: Add the import and OnboardingChecklist component**

Add to the top of `Dashboard.tsx`, after the existing imports:

```typescript
import { useOnboarding } from "../contexts/OnboardingContext";
import { useNavigate as useNav } from "react-router";
import { CheckCircle2, Circle, ChevronRight, X as XIcon } from "lucide-react";
```

Note: `useNavigate` is already imported as `useNavigate` in this file. Add the `CheckCircle2, Circle, ChevronRight, X as XIcon` to the existing lucide-react import line instead of adding a new import. The existing line is:
...
- [ ] **Step 2: Add the checklist widget inline in Dashboard**

Inside `export default function Dashboard()`, after the existing `const { isExecutiveMode }` line, add:
...
Then, inside the returned JSX, add the checklist widget **immediately before the KPI strip** `{/* KPI Strip */}` comment. Insert:
...
git commit -m "feat: add onboarding checklist widget to Dashboard (shown post-import)"
```

---

## Task 10: App.tsx — Wrap with OnboardingProvider + final integration verification

**Files:**
- Modify: `src/app/App.tsx`

- [ ] **Step 1: Add OnboardingProvider to the provider tree**

Find the current `App.tsx`:
...
Expected: `✓ built in` with no TypeScript errors. (A chunk size warning about the main bundle is expected and not an error.)
...
1. Navigate to http://localhost:5176/settings/data-sources
2. Click "Import New Portfolio" — wizard opens
3. Step 1: Click "Download template (.xlsx)" — browser downloads aeroinsights-import-template.xlsx
4. Click "I have my file ready"
5. Step 2: Drag the downloaded template onto the drop zone (or click to browse and select it)
6. Click "Validate file"
7. Step 3: Validation runs (2–3 seconds). Errors shown (empty lessee_name in row 2 note row,
   or all-clear if sample data is valid). Summary shows row count, lessees, errors, warnings.
8. If all-clear: click "Preview data"
9. Step 4: Preview shows 2 sample rows from template, KPI grid (2 leases, 2 lessees, 2 types, date range)
10. Click "Confirm & run ECL"
11. Step 5: Progress bar animates through 6 phases (~6 seconds). ECL value appears.
12. Click "Go to Dashboard" — navigates to /
13. Dashboard: Onboarding checklist widget visible above KPI strip (4 items, 0/4 done)
14. Click a checklist item — navigates to target route, item marked complete on return
15. Click X on checklist — checklist dismissed, does not reappear on refresh
    (Verify: refresh page — checklist still dismissed, because localStorage persists state)
```
...
git commit -m "feat: wrap App with OnboardingProvider — P1-F1 Data Import Wizard complete"
```
...
|---|---|
| Settings > Data Sources > "Import New Portfolio" entry point | Task 8 |
| Step 1: Download template with 6 sheets | Task 3 — `TEMPLATE_SHEETS` array, `generateTemplate()` |
| Step 1: Column header tooltips (required/optional, example, note) | Task 3 — row 2 of each sheet is a notes/tooltip row |
| Step 2: Drag-and-drop CSV/XLSX upload | Task 4 — `Step2` component |
| Step 2: Row count per entity type | Task 5 — summary KPI row in Step3 |
| Step 2: Detected errors (missing fields, date formats, unknown aircraft) | Task 5 — `parseAndValidate()` |
| Step 2: Warning flags (MR rate = 0, missing MSN) | Task 5 — warnings in `parseAndValidate()` |
| Step 2: "Fix now" inline editor | Task 5 — inline `<input>` per error row in Step3 |
| Step 3: Preview — 5 sample leases | Task 6 — `Step4` sample table |
| Step 3: Lessee count, aircraft types, date range | Task 6 — `Step4` KPI grid |
| Step 4: Confirm & Run Initial ECL | Task 6 — `Step5` with 6-phase animation |
| Step 4: Target ≤15 min raw upload to ECL output | Simulation completes in ~6 seconds |
| Step 5: Post-import checklist on Dashboard | Tasks 1, 9 |
| Checklist items: SICR triggers, scenario weights, review lessees, stress test | Task 1 — `DEFAULT_CHECKLIST` |
| Checklist dismissable | Task 9 — X button calls `dismissChecklist()` |
| Checklist persists across refreshes | Task 1 — localStorage via `STORAGE_KEY` |
...
- `ValidationResult` defined in Task 2, consumed in Tasks 5 (Step3), 6 (Step4, Step5), and Task 7 (shell state). ✓
- `ParsedRow._rowIndex` used in errors/warnings and preview table. ✓
- `useOnboarding()` returns `{ markImportComplete, toggleItem, dismissChecklist, isNewTenant, checklist, checklistDismissed }` — all field names consistent across Tasks 1, 6, 9. ✓
- `TEMPLATE_SHEETS` type uses `headers: Array<{ col, required, example, note }>` — consistently destructured in `generateTemplate()` and Step1 render. ✓
