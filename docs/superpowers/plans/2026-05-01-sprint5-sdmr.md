# Sprint 5 — SD / MR Logic Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a new "SD / MR" tab to the Portfolio page showing per-lease security deposit records, maintenance reserve ledgers by component, refund cap enforcement, EOL cash compensation toggle, and LGD offset display.

**Architecture:** Single new file `src/app/components/portfolio/SDMRTab.tsx` holds all types, data, computed functions, and UI. `Portfolio.tsx` gets a one-line tab addition and import. Follows the same isolation pattern as `IAS36Tab.tsx`.

**Tech Stack:** React 18, TypeScript, inline styles (`style={{}}`), existing `<Card>`, `<KpiCard>`, `<StatusPill>` components.

---

## Task 1: Types, data, computed functions, shell component

**Files:**
- Create: `src/app/components/portfolio/SDMRTab.tsx`

- [ ] **Step 1: Create the file with all interfaces, the full dataset, all pure computed functions, and a shell component that passes `npm run build`**

Create `/Users/tanamsethi/Downloads/Aeroinsights/src/app/components/portfolio/SDMRTab.tsx`:
...
git commit -m "feat(sprint5): add SDMRTab types, data, and computed functions"
```

---

## Task 2: KPI strip + accordion table (collapsed rows)

**Files:**
- Modify: `src/app/components/portfolio/SDMRTab.tsx`

Replace the `SDMRTab` function (from `export function SDMRTab()` to end of file) with:

- [ ] **Step 1: Replace shell component with KPI strip and accordion table**
...
export function SDMRTab() {
  ...
  const wtdLGDReduction = sdmrData.reduce((s, l) => {
    const w = l.eadNum / sdmrData.reduce((a, x) => a + x.eadNum, 0);
                    ...
                    {isOpen && (
                      <tr key={`${lease.leaseId}-detail`} style={{ borderBottom: "1px solid #E2E8F0" }}>
                        <td colSpan={10} style={{ padding: "0", background: "#FAFAFA" }}>
                          {/* Expanded panel rendered in Task 3 */}
...
cd /Users/tanamsethi/Downloads/Aeroinsights
git add src/app/components/portfolio/SDMRTab.tsx
git commit -m "feat(sprint5): add KPI strip and accordion table collapsed rows"
```

---

## Task 3: Expanded panel — SD card, MR ledger, LGD offset breakdown

**Files:**
- Modify: `src/app/components/portfolio/SDMRTab.tsx`

Replace the placeholder `Detail panel — implemented in Task 3` `<div>` with the full three-section expanded panel. Replace this exact block:
...
- [ ] **Step 1: Replace placeholder with three-section expanded panel**

```tsx
                          <ExpandedPanel lease={lease} condition={conditions[lease.leaseId]} />
```

Then add the `ExpandedPanel` component **above** `SDMRTab` (after the computed functions):
...
cd /Users/tanamsethi/Downloads/Aeroinsights
git add src/app/components/portfolio/SDMRTab.tsx
git commit -m "feat(sprint5): add expanded SD card, MR ledger, and LGD offset panel"
```

---

## Task 4: ECL feedback table + wire into Portfolio.tsx

**Files:**
- Modify: `src/app/components/portfolio/SDMRTab.tsx` — add ECL feedback table at bottom of `SDMRTab`
- Modify: `src/app/pages/Portfolio.tsx` — add tab + import + render

- [ ] **Step 1: Add ECL feedback table to SDMRTab**

Inside `SDMRTab`, after the closing `</Card>` of the accordion (just before the final closing `</div>`), add:
              ...
              {sdmrData.map((lease, i) => {
                ...
                return (
                  <tr key={lease.leaseId} style={{ borderBottom: "1px solid #F1F5F9", background: i % 2 === 0 ? "#FFFFFF" : "#F4F5F7" }}>
                    ...
                    <td style={{ padding: "0.75rem 1rem", color: "#15803D", fontWeight: 500 }}>{(optimisticOffset(lease) * lease.baseLGD < lease.baseLGD ? Math.max(0, lease.baseLGD * (1 - optimisticOffset(lease))).toFixed(1) : "0.0")}%</td>
                    <td style={{ padding: "0.75rem 1rem", fontWeight: 600, color: "#15803D", fontVariantNumeric: "tabular-nums" }}>
                      {eclDelta.toFixed(2) === "-0.00" ? "—" : `${eclDelta >= 0 ? "+" : ""}$${Math.abs(eclDelta).toFixed(2)}M`}
                    </td>
                  </tr>
                );
              })}
...
- [ ] **Step 2: Wire SDMRTab into Portfolio.tsx**

Open `src/app/pages/Portfolio.tsx`.
...
```tsx
const tabs = ["Leases", "Aircraft", "Lessees", "Concentration"];
```
...
Add the render condition just before the closing `</div>` of the page:
...
```bash
cd /Users/tanamsethi/Downloads/Aeroinsights
git add src/app/components/portfolio/SDMRTab.tsx src/app/pages/Portfolio.tsx
git commit -m "feat(sprint5): add ECL feedback table and wire SD/MR tab into Portfolio page"
```
