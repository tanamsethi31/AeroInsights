...
2. **New pure function: `computeLiveCalibration(raw: LiveMacroData): ScenarioCalibrationDivergence[]`** in `src/app/data/intelligenceData.ts` (co-located with the existing static `SCENARIO_CALIBRATION` and its type, since it produces the same shape).

   **Correction found while planning:** the static array has 3 entries, but only 2 are honestly live-mappable. `cal-02` (India GDP, ← `gdp` entry where `countryCode === "IND"`) and `cal-03` (EUR/USD Rate, ← `eurUsd`) are direct unit matches — the live feed and the static baseline are the same metric in the same unit, so the divergence is a straightforward subtraction. `cal-01` (Jet-A1 Fuel Price, $/litre) has **no genuine live match**: the only live fuel-adjacent feed is Brent crude ($/barrel) from `api/signals/macro.ts`, a different (correlated but not equivalent) commodity. Producing a "$/litre jet fuel" figure from Brent crude would require a crack-spread conversion constant that doesn't exist anywhere in this codebase — inventing one would mean fabricating a number to make the calibration banner *look* live, which defeats the entire purpose of this pass. **`cal-01` stays static/reference in this pass.** A genuine fix would need a real jet-fuel price feed (e.g. IATA Jet Fuel Price Monitor), which is out of scope here and not something to fake in the meantime.

   For the 2 live-mappable entries, compute `currentMarket` (formatted from the live number + a live-source citation, e.g. `` `5.8% growth (IMF WEO, FY2026)` `` for GDP, `` `1.0789 (ECB, 29 Apr 2026)` `` for FX), `divergence` (difference from the existing `scenarioAssumption` baseline — the baseline values themselves stay hardcoded strings, since they represent the model's last-run assumption, a fixed reference point that doesn't need to be live), and `suggestedValue` (the raw delta, same units as today: `gdpDelta` as a decimal, `fxDelta` as a decimal fraction). `cal-01` always passes through unchanged from `SCENARIO_CALIBRATION`.

   Severity bands (mirrors the existing hand-assigned values — GDP 0.6pp→high, FX 2.0%→medium):
